...
public class TGQ_New_Quote_Page extends PTGQAllPages {
	...
	public void newQuote() throws MyOwnException, InterruptedException {
		...
		try {
			...
			if (mywebEle.isElementExistwithid("referralLocationCodeInputField"))
				mywebEle.enterText(tgq_zip_code, currentHash.get("ZipCode"));
			else if (mywebEle.isElementExistwithid("enhancedContinueInsured")
					&& !(enhancedTransfer.getAttribute("style").equals("display: none;"))) {
				mywebEle.enterText(zipInGarage, currentHash.get("ZipCode"));
			...
			myWait.until(ldriver, "ELEMENT_CLICKABLE", 30, tgq_next);
			/*
			 ...
			 * "New Quote Page", "./Results/Screenshots_" + testCaseNumber+ testRunTimeStamp
			 * + "/" + "New Quote Page.png");
			 */
			mywebEle.clickOn(tgq_next);

		} catch (Exception exp) {
			log.error(exp.getMessage());
			/*
			 * base.screenShot("./Results/Screenshots_" + testCaseNumber+ testRunTimeStamp +
			 * "/" + "Error in Opening New Quote Page.png");
			 * reportVar.logTestCaseStatusWithSnapShot(parentTestCase, "FAIL",
			 * "<font color=red><b>Error while Opening New Quote Page: </b></font><br />" +
			 * exp.getMessage() + "<br />", "./Results/Screenshots_" + testCaseNumber+
			 * testRunTimeStamp + "/" + "Error in Opening New Quote Page.png");
			 */
