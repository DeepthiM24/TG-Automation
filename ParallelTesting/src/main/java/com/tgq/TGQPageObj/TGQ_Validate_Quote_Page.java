...
public class TGQ_Validate_Quote_Page extends PTGQAllPages {
	...
	public void validateQuote() throws MyOwnException, InterruptedException {
		...
		try {
			...
			if (mywebEle.isElementExistwithid("quoteBean.primaryAccountAlertsOptInPreference.writableValue")) {
			...
			if (mywebEle.isElementExistwithid("quoteBean.accessEmail.value")&& !(emailRow.getAttribute("style").equals("display: none;"))) {
				mywebEle.enterText(confirm_email, currentHash.get("Emailid"));
			}
			/*
			 * base.screenShot("./Results/Screenshots_" + testCaseNumber+ testRunTimeStamp +
			 * "/" + "ValidateQuote Tab.png");
			 * reportVar.logTestCaseStatusWithSnapShot(parentTestCase, "PASS",
			 * "ValidateQuote", "./Results/Screenshots_" + testCaseNumber+ testRunTimeStamp
			 * + "/" + "ValidateQuote Tab.png");
			 */
			mywebEle.clickOn(tgq_next_btn);
			Thread.sleep(4000);
		} catch (Exception exp) {
			log.error(exp.getMessage());
			/*
			 * base.screenShot("./Results/Screenshots_" +testCaseNumber+ testRunTimeStamp +
			 * "/" + "Error in Opening ValidateQuote Tab.png");
			 * reportVar.logTestCaseStatusWithSnapShot(parentTestCase, "FAIL",
			 * "<font color=red><b>Error while Opening ValidateQuote: </b></font><br />" +
