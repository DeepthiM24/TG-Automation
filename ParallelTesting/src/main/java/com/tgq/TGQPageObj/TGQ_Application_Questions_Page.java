...
public class TGQ_Application_Questions_Page extends PTGQAllPages {
	...
	public void applicationquestions() throws  InterruptedException, MyOwnException {
		...
		try {
			if (!(currentHash.get("QuoteState").equals("CO"))) {
				...
				if (!currentHash.get("PolicyType").equals("Bond - No Credit")) {
					if (mywebEle.isElementExistwithid("quoteBean.appQuestions[1].writableValue")) {
						Select appQue_2 = new Select(appQuestions_2);
						appQue_2.selectByVisibleText("No");
					}
				} else {
					if (mywebEle.isElementExistwithid("quoteBean.appQuestions[2].writableValue")) {
				...
				if (mywebEle.isElementExistwithid("quoteBean.appQuestions[6].writableValue")) {
			 ...
			 * "/" + "Application questions Tab.png");
			 * reportVar.logTestCaseStatusWithSnapShot(parentTestCase,
			 * "PASS","Opened Application questions Tab","./Results/Screenshots_" +
			 * testCaseNumber+ testRunTimeStamp + "/" + "Application questions Tab.png");
			 */
			next_button.click();
			Thread.sleep(4000);
		} catch (Exception exp) {
			 ...
			 * "<font color=red><b>Error while Opening Application questions Tab: </b></font><br />"
			 * + exp.getMessage() + "<br />", "./Results/Screenshots_"+ testCaseNumber+
			 * testRunTimeStamp + "/" + "Error in Opening Application questions Tab.png");
			 */
